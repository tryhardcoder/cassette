...
void blu_makeDrawCalls(UniBlock** listHead);
...
// CLEANUP: that lol ^^^^^^^^^^^^^^^^^^^^^
void _blu_renderString(str string, V2f start, float scale, Rect2f clip, V4f color, UniBlock** listHead) {
    ...
    for(int i = 0; i < string.length; i++) {
        ...
        UniBlock* block = registerCall(listHead, &globs.frameArena);

        block->color = color;
        block->fontTextureId = globs.fontTex->id;
        block->textureId = globs.solidTex->id;
...
void _blu_genRenderCallsRecurse(blu_Area* area, UniBlock** listHead) {
    ...
    while(area) {
        ...
        if(area->flags & blu_areaFlags_DRAW_BACKGROUND) {
            ...
            block->fontTextureId = globs.solidTex->id;
            block->textureId = globs.solidTex->id;
        ...
        if (area->flags & blu_areaFlags_DRAW_TEXTURE) {
            ...
            block->textureId = area->texture->id;
            block->fontTextureId = globs.solidTex->id;
            block->clipStart = area->clipRect.start;
            block->clipEnd = area->clipRect.end;
            block->cornerRadius = area->style.cornerRadius;
        ...
        _blu_genRenderCallsRecurse(area->firstChild[globs.linkSide], listHead);
        area = area->nextSibling[globs.linkSide];
    }
}
void blu_makeDrawCalls(UniBlock** listHead) {
    _blu_genRenderCallsRecurse(globs.ogParent, listHead);
    _blu_genRenderCallsRecurse(globs.cursorParent, listHead);
}
